...
export function TransactionList() {
  ...
  const filterTransaction = useCallback(
    (transaction: Transaction, filters: QuickFilterState): boolean => {
      // Фильтр по типу
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">
              Показано: {transactionStats.count} из {state.transactions.length}{" "}
              транзакций
            </p>
          </div>
        </div>
          ...
          {filteredTransactions.length === 0 ? (
            <div className="text-center py-12">
              ...
              {activeFiltersCount > 0 ? (
                ...
                <Button onClick={handleShowForm}>Добавить транзакцию</Button>
              )}
            </div>
          ) : (
